...
"""This node responds with an explanation for the input image in Japanese.

Parameters:
    input_topic_name: 'input_topic' (default: 'image_raw')
    output_topic_name: 'picture_explanation' (Explanation of captured image)
    debug_output_topic_name: 'picture_explanation/debug_image' (Captured image)
...
class PaliGemmaExplainNode(Node):
    """Node for explaining images using PaliGemma model.

    This node subscribes to an image topic, processes the image using the PaliGemma
    model, and publishes both the explanation and the processed image.
    """
    ...
    def __init__(self):
        ...
        self._processed_img_pub = self.create_publisher(
            Image, 'picture_explanation/debug_image', 5
        )
    ...
    def _image_callback(self, msg: Image) -> None:
        """Process incoming image and publish results.
